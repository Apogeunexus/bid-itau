...
// ---------------------------------------------------------------------------
// Os denominadores das faixas de preferência
// ---------------------------------------------------------------------------

/**
 * Os números que as cinco faixas declaram, contados ITEM A ITEM.
 *
 * ESTE BLOCO EXISTE POR CAUSA DE UM DEFEITO, e ele é uma classe, não um caso. As três
 * primeiras faixas somavam FACETAS para achar quantos itens declaram linguagem — e um
 * podcast que declara literatura e música entra nas duas. O HTML saiu do build dizendo
 * «50 dos 336 podcasts não declaram linguagem» quando são 100, e «33 das 54 formações
 * declaram» quando são 24. Números plausíveis, errados, e na cara do avaliador.
 *
 * Faceta conta DECLARAÇÕES. Quando a pergunta é «quantos itens», a resposta se conta nos
 * itens. Os valores moram aqui, num lugar só, e o gate confere cada um contra o acervo —
 * era isso que faltava para o erro não ter passado.
 */
export interface MedidasDasFaixas {
  /** As 529 mídias do acervo inteiro. */
  midias: number;
  /** Quantas delas são podcast e moram no Cast. */
  podcasts: number;
  /** Dos podcasts, quantos não declaram linguagem nenhuma. */
  podcastsSemLinguagem: number;
  /** As formações da Escola. */
  formacoes: number;
  /** Quantas delas declaram alguma linguagem. */
  formacoesComLinguagem: number;
  /** Temas com matéria editorial atrás. */
  temasComMateria: number;
}

export function medidasDasFaixas(): MedidasDasFaixas {
  const midias = slugsPorTipo("midia")
    .map((slug) => porSlug("midia", slug))
    .filter((e): e is NonNullable<typeof e> => Boolean(e));
  const podcasts = midias.filter((m) => m.extra?.categoria === "podcasts");
  const formacoes = slugsPorTipo("formacao")
    .map((slug) => porSlug("formacao", slug))
    .filter((e): e is NonNullable<typeof e> => Boolean(e));

  return {
    midias: midias.length,
    podcasts: podcasts.length,
    podcastsSemLinguagem: podcasts.filter((m) => m.linguagens.length === 0).length,
    formacoes: formacoes.length,
    formacoesComLinguagem: formacoes.filter((f) => f.linguagens.length > 0).length,
    temasComMateria: totalDeTemasComMateria(),
  };
}
