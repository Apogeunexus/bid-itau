...
export const REGRAS: Regra[] = [
  ...
  {
    /**
     * Percorrer uma exposição inteira não valia nada, e o evento já existia no
     * vocabulário desde o começo: `museu.exposicao.percorrida` estava declarado em
     * `tipos.ts` sem nenhuma regra do outro lado. Uma visita ao museu virtual paga
     * abaixo de um curso e acima de uma matéria, que é onde ela cai em esforço.
     */
    id: "r-exposicao",
    versao: 1,
    descreve: "Percorrer uma exposição do museu virtual até o fim",
    quando: "museu.exposicao.percorrida",
    maxPorDia: 3,
    entao: [
      { conceder: { ativo: "percurso", valor: 40, motivo: "Exposição percorrida" } },
      { conceder: { ativo: "ficha", valor: 5, motivo: "Exposição percorrida" } },
    ],
  },
  {
    /**
     * Mesmo caso: `perfil.completo` existia no tipo e não pagava nada. Paga uma vez
     * por pessoa — perfil não se completa duas vezes, e `maxPorPersona` é o que
     * impede que editar a foto vire fonte de fichas.
     */
    id: "r-perfil",
    versao: 1,
    descreve: "Completar o perfil",
    quando: "perfil.completo",
    maxPorPersona: 1,
    entao: [
      { conceder: { ativo: "percurso", valor: 40, motivo: "Perfil completo" } },
      { conceder: { ativo: "ficha", valor: 5, motivo: "Perfil completo" } },
    ],
  },
  {
    id: "r-disposicoes",
...
export const MISSOES: MissaoDefinida[] = [
  ...
  },
  {
    ...
    descricao: "Curta uma publicação, responda alguém e guarde um post para depois.",
    alvo: 3,
    avancaCom: [
